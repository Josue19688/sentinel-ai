...
# ── Instancia compartida del motor de riesgo ──────────────────────────────────
# Síncrona porque Celery no usa asyncio event loops.
# Una sola instancia por worker process (no por tarea).

_risk_engine = RiskEngine(db_url=settings.DATABASE_URL)


def get_risk_engine() -> RiskEngine:
    """Devuelve la instancia compartida del motor de riesgo."""
    return _risk_engine
